...
HIDDEN_UNIT_COUNT = OUTPUT_RANK * 2         # How many hidden units are there inside a RNN cell
...
def rnn(inputs, seq_len_of_inputs, weights, biases, batch_size=1):
    ...
    cell_outputs, cell_states = tf.nn.dynamic_rnn(cell, cell_inputs, sequence_length=seq_len_of_inputs, initial_state=cell_initial_state)
    cell_c_states, cell_m_states = cell_states
    results = tf.matmul(cell_m_states, weights['out']) + biases['out']
    return results

pred = rnn(x, seq_len_of_x, weights, biases, TRAIN_BATCH_SIZE)
